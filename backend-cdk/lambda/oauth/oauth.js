// oauth.js
// CDK版 Slack OAuth用エンドポイント (Node.js 22)
...
const SLACK_REDIRECT_URI = process.env.SLACK_REDIRECT_URI || secrets.SLACK_REDIRECT_URI;
...
app.get("/slack/oauth/callback", async (req, res) => {
  ...
  try {
    ...
    // Slack OAuth v2 APIを呼び出してアクセストークンを取得
...
app.get("/slack/oauth/workspaces", async (req, res) => {
  try {
    console.log("DynamoDBからワークスペース一覧を取得中");
    const params = {
      TableName: DYNAMODB_TABLE,
    ...
    console.log("取得したワークスペース:", workspaces);
    res.status(200).json({ done: true, data: workspaces });
  } catch (e) {
    console.error("ワークスペース取得エラー:", e);
...
app.get("/slack/oauth/channels", async (req, res) => {
  ...
  try {
    ...
    // 取得したアクセストークンでSlack APIからチャンネル一覧を取得
